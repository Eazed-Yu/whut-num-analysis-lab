...
import { newtonInterpolation } from '../core/chapter2'
...
const calculateInterpolation = () => {
  ...
  try {
    state.result = newtonInterpolation({ xs, ys, x: state.targetX })
  } catch (error) {
...
const chartOption = computed(() => {
  ...
  if (xs.length >= 2) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      try {
        const y = newtonInterpolation({ xs, ys, x })
        curveData.push([x, y])
      } catch (e) {
        // 跳过可能产生错误的点
        continue
    ...
    series: [
      {
        ...
        data: curveData,
        showSymbol: false,
        smooth: false, // 牛顿插值曲线已经计算好了，不需要额外的平滑
        lineStyle: {
          width: 2,
          color: 'purple',
        },
        ...
        <div class="interpolation-section">
          ...
          <div class="result" v-if="state.result !== null">
            <p>
              牛顿插值结果:
              <br />
              N({{ state.targetX }}) = <strong>{{ state.result.toFixed(6) }}</strong>
            </p>
          </div>
        </div>
      </div>
