...
const calculateInterpolation = () => {
  ...
  try {
    result.value = newtonInterpolation({ xs, ys, x: targetX.value })
  } catch (error) {
  ...
  <div class="newton-container">
    ...
    <div class="form-section">
      ...
      <div class="result" v-if="result !== null">
        <p>
          牛顿插值结果 N({{ targetX }}) = <strong>{{ result.toFixed(6) }}</strong>
        </p>
      </div>
    </div>
    ...
    <div class="chart-container">
      ...
      <svg :width="chartWidth" :height="chartHeight" class="chart">
        ...
        <!-- 插值曲线 -->
        <path :d="curvePath" fill="none" stroke="purple" stroke-width="2" />
